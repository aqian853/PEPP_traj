...
gmm2_2_cdss_18 <- readRDS("gmm2_2_CDSS_18.rds")
gmm3_2_cdss_18 <- readRDS("gmm3_2_CDSS_18.rds")
gmm2_2_sans_18 <- readRDS("gmm2_2_sans_18.rds")
gmm3_2_sans_18 <- readRDS("gmm3_2_sans_18.rds")
gmm2_2_LV1_18 <- readRDS("gmm2_2_LV1_18.rds")
gmm3_2_LV1_18 <- readRDS("gmm3_2_LV1_18.rds")
gmm2_sofas <- readRDS("gmm2_2_sofas.rds")

## Extracting pprob
div_cdss_2 <- gmm2_2_cdss_18[["pprob"]]
div_cdss_2 <- div_cdss_2 %>%
  rename(class_cdss_2 = class,
         prob1_cdss_2 = prob1,
         prob2_cdss_2 = prob2
  )
div_cdss_3 <- gmm3_2_cdss_18[["pprob"]]
...
div_sans_2 <- gmm2_2_sans_18[["pprob"]]
div_sans_2 <- div_sans_2 %>%
  rename(class_sans_2 = class,
         prob1_sans_2 = prob1,
         prob2_sans_2 = prob2
  )
div_sans_3 <- gmm3_2_sans_18[["pprob"]]
...
div_LV1_2 <- gmm2_2_LV1_18[["pprob"]]
div_LV1_2 <- div_LV1_2 %>%
  rename(class_LV1_2 = class,
         prob1_LV1_2 = prob1,
         prob2_LV1_2 = prob2,
  )
div_LV1_3 <- gmm3_2_LV1_18[["pprob"]]
div_LV1_3 <- div_LV1_3 %>%
...
gmm_class_div_18mo <- full_join(div_cdss_joint, div_sans_joint, by = "pin") %>%
  full_join(div_LV1_joint, by = "pin")

gmm_class_div_18mo <- gmm_class_div_func_18mo %>% # Remove NAs
  filter(!(is.na(class_cdss_2) & is.na(class_sans_2)))

gmm_class_div_func_18mo <- full_join(div_cdss_joint, div_sans_joint, by = "pin") %>%
  full_join(div_sofas_2, by = "pin") %>%
  full_join(div_LV1_joint, by = "pin")

gmm_class_div_func_18mo <- gmm_class_div_func %>% # Remove NAs
  filter(!(is.na(class_cdss_2) & is.na(class_sans_2) & is.na(class_sofas_2)))

## Creating a contingency table
## Adjust class and variable here
clust1 <- gmm_class_div_func_18mo$class_sofas_2 ## Vertical
clust2 <- gmm_class_div_func_18mo$class_LV1_3 ## Horizontal
...
mat <- table(paste0("SOFAS_", clust1), paste0("LV1_", clust2))
